...
| **PAST x m SENSOR LIMIT — ASCEND (red)** | At/past the sensor's maximum depth: depth data may be unreliable from here — ascend. A red warning line at the bottom of the dive display with a pulsing red background and a repeating strong buzz; the dive data itself (depth, stops, TTS) stays visible, flagged rather than hidden. Warned in amber from 90 % of the limit (“NEAR x m SENSOR LIMIT”). |
